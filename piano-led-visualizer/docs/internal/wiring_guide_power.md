...
1. **下から1行目を「GNDバス（黒）」、下から3行目を「VCCバス（赤）」にする**:
   - **重要：間に1行（2行目の穴）を空の状態にしておく**。
   - 理由は、はんだを盛ってレールを作る時に、隣のレールと誤ってはんだがくっつかないようにするため（ショート防止）です。
...
## 4. ⚠️ 重要な注意：コネクタの電流限界 (ボトルネック)

今回使用する **JST-SM 3P コネクタ** の定格電流は、一般的に1ピンあたり **最大3A** です。
一方、ACアダプタは **最大6.2A** 流せる能力があり、ヒューズは **10A** を想定しています。

- **リスク**: LEDを「全点灯・最大輝度（真っ白）」にすると、3Aを超えてコネクタ部分が熱を持ち、最悪の場合プラスチックが溶ける恐れがあります。
- **対策**:
    1. **ソフト側での制限**: プログラム側で全体の輝度を50%程度に制限すれば、実用上の電流は3A以下に収まります。ピアノの視認用であれば、これで十分実用になります。
    2. **ハード側の工夫**: もし高輝度で使いたい場合は、10A対応の別の極太コネクタに変えるか、LEDテープの最初と最後（両端）から別々に電源を供給して電流を分散させる必要があります。

まずは「ソフト側で輝度を欲張りすぎない」ことを念頭に置いて進めましょう！😊🛡️⚖️
