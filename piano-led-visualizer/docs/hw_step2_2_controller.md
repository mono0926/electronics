...
#### 🕹 オプション：ジョイスティック・ボタン操作の有効化
液晶HATのボタンやスティック本体で操作したい場合は、以下の信号線も追加でブリッジしてください。

| 信号 | 番号 | **Pi側 (垂直・左端)** | **LCD側 (水平・上端)** | 役割 |
| :--- | :---: | :--- | :--- | :--- |
| **UP** | **31** | **右列・下16番目** | **上段・左16番目** | スティック（上） |
| **DOWN** | **35** | **右列・下18番目** | **上段・左18番目** | スティック（下） |
| **LEFT** | **29** | **右列・下15番目** | **上段・左15番目** | スティック（左） |
| **RIGHT** | **37** | **右列・下19番目** | **上段・左19番目** | スティック（右） |
| **PRESS** | **33** | **右列・下17番目** | **上段・左17番目** | スティック（押込） |
| **KEY1** | **40** | **左列・下20番目** | **下段・左20番目** | 右ボタン（決定） |
| **KEY2** | **38** | **左列・下19番目** | **下段・左19番目** | 右ボタン（戻る） |
| **KEY3** | **36** | **左列・下18番目** | **下段・左18番目** | 右ボタン（切替） |
