...
## 3. ディスプレイ (LCD HAT) の装着

ステータス（IPアドレスやCPU使用率）を表示するために Waveshare 1.44" LCD HAT を使います。

### 🥪 スタッキング（重ね合わせ）のコツ

ラズパイの上に直接LCDを載せると、先ほど作った配線基板を挟む隙間がありません。ここで**連結ピンソケット（スタッキングピン）**を使います。

1. **配線基板を通す**: ラズパイのピンヘッダに、配線済みのユニバーサル基板を差し込みます。
2. **スタッキングピンの挿入**: その上から連結ピンソケットを差し込み、ピンを延長します。
3. **LCDの装着**: 延長されたピンの上に LCD HAT を載せます。

これで、内部にレベルシフタを隠しながら、最上段でディスプレイを確認できる「三層構造」の完成です！
...
- [ ] 三層構造（ラズパイ ＋ 配線基板 ＋ LCD）がガタつかずに固定されている。
