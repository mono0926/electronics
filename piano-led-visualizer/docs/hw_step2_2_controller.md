# フェーズ 2.2: コントローラー（ラズパイ・レベルシフタ）の配線 🧠⚡️

このフェーズでは、Raspberry Pi からの信号をLEDテープが理解できる電圧（5V）に変換し、ディスプレイとともに組み立てる工程を解説します。

---

## 🛠 準備するもの

- [ ] **Raspberry Pi Zero 2 WH** (ピンヘッダ実装済み)
- [ ] **SN74AHCT125N** (レベルシフタ IC)
- [ ] **Waveshare 1.44" LCD HAT**
- [ ] **ユニバーサル基板 (大)** (例: 秋月 100518 95×72mm)
- [ ] **ピンヘッダ 1×40** (適宜カット) または連結ピンソケット
- [ ] **ICソケット 14P** (ICの保護用)
- [ ] **330Ω 抵抗**
- [ ] **ジャンパー線 / シリコンワイヤー (24AWG)**

---

## 1. なぜレベルシフタが必要なのか？

Raspberry Pi の信号（GPIO）は **3.3V** で出力されます。しかし、WS2812B LEDテープは **5V** の信号を期待しています。
3.3Vでも「たまたま動く」ことはありますが、動作が不安定になったり、LEDが勝手に点滅したりする原因になります。**SN74AHCT125** を使うことで、3.3Vをクリーンな5Vへと「翻訳」し、安定した制御を実現します。

---

## 2. 信号変換回路の作成

ICを直接基板に立てるのではなく、**ICソケット**を使うことを強く推奨します。

### 📌 SN74AHCT125 ピン配置と接続先

```mermaid
graph TD
    subgraph RPi["Raspberry Pi (3.3V)"]
        G18["GPIO 18 (Pin 12)"]
        RPiGND["GND (Pin 6/14/...)"]
        RPi5V["5V (Pin 2/4)"]
    end

    subgraph IC["74AHCT125 (Level Shifter)"]
        VCC["Pin 14 (VCC)"]
        GND_IC["Pin 7 (GND)"]
        A1["Pin 2 (1A 入力)"]
        Y1["Pin 3 (1Y 出力)"]
        OE1["Pin 1 (1OE イネーブル)"]
    end

    subgraph LED["LED Strip (5V)"]
        DIN["DIN (Data In)"]
    end

    G18 --> A1
    RPi5V --> VCC
    RPiGND --> GND_IC
    RPiGND --> OE1
    Y1 -->|330Ω 抵抗を挟む| DIN
```

### 🔨 組み立ての手順

1. **ICソケットの配置**: ユニバーサル基板の中央付近にICソケットを配置します。
2. **電源供給**:
   - ICの **Pin 14** をラズパイの **5V** ピンへ接続。
   - ICの **Pin 7** をラズパイの **GND** ピンへ接続。
3. **信号入力**: ラズパイの **GPIO 18 (Pin 12)** を ICの **Pin 2 (1A)** へ接続。
4. **イネーブル設定**: ICの **Pin 1 (1OE)** を **GND** に落とします。 これをしないと出力が有効になりません。
5. **出力（信号の出口）**: ICの **Pin 3 (1Y)** から **330Ωの抵抗** を介して、LEDテープのコネクタ（`DIN`）へ繋ぎます。
   - _※ 330Ωの抵抗は、信号の反射を防ぎ、LEDの最初のチップを電気的な衝撃から守る「保護装置」として機能します。_

---

## 3. ディスプレイ (LCD HAT) の装着とレイアウト

ステータス（IPアドレスやCPU使用率）を表示するために Waveshare 1.44" LCD HAT を使います。
高負荷になる本プロジェクトにおいて熱暴走を防ぐため、**「横並び（案B）」のレイアウトを強く推奨**します。

> [!NOTE]
> **独自アレンジについて**
> ベースプロジェクト（[`onlaj`版](https://github.com/mono0926/Piano-LED-Visualizer/tree/ja)）では極小サイズを目指しラズパイの上に基板を重ねる「三層構造」が基本ですが、本プロジェクトでは熱対策とメンテナンス性を高めるため、大型基板の上で平面的に配置する「横並び構成」を独自に推奨しています。

### 🥪 横並び配置（推奨）のコツ

大型のユニバーサル基板（90×70mm程度）を使用し、Raspberry Pi と LCD を平面的に並べます。

1. **Pi Zero の固定**: 基板の端にピンヘッダ（または連結ピンソケット）を立てて Pi Zero を固定します。
2. **LCD の固定**: Pi に被さらない位置に LCD HAT を並べて配置し、ピンヘッダで固定します。
3. **部品の配置**: 空いたスペースにレベルシフター（ICソケット）、抵抗、コンデンサなどを配置します。
4. **配線**: 裏面でそれぞれのピン（5V, GND, GPIO等）をジャンパワイヤやスズメッキ線で繋ぎます。

### 🎨 LCDへのブリッジ配線（全9本）
本プロジェクトでは、熱対策とスペースの都合上、**「ラズパイ垂直配置 ⇔ LCD水平配置」** の 90 度オフセットレイアウトを採用しています。

#### 裏面（半田面）から見た空間の定義
（※SDカードスロット側を「下」にした状態）
- **ラズパイ**: 基板の **左側** に垂直配置。
    - **左列(外)** = 偶数ピン (2, 4, 6...)
    - **右列(内)** = 奇数ピン (1, 3, 5...)
- **LCDヘッダ**: 基板の **上端** に水平配置。
    - **下段(内)** = 偶数ピン (2, 4, 6...)
    - **上段(外)** = 奇数ピン (1, 3, 5...)

### 🎨 LCD・操作系へのフルブリッジ配線（全17本）

「横並び（案B）」において、表示と物理操作の両方を有効にするための確定マップです。
後から線を追加するのは大変ですので、**この17本をまとめて配線することを確認推奨**します。

| 信号 | 番号 | **Pi側 (垂直・左端)** | **LCD側 (水平・上端)** | **推奨色** | 区分 |
| :--- | :---: | :--- | :--- | :---: | :--- |
| **5V** | **2** | **左列・一番下** | **下段・一番左** | **赤** | 電源 |
| **GND** | **6** | **左列・下3番目** | **下段・左3番目** | **黒** | 電源 |
| **3.3V** | **1** | **右列・一番下** | **上段・一番左** | **黄** | 電源 |
| **RST** | **13** | **右列・下7番目** | **上段・左7番目** | **緑** | 表示 |
| **BL** | **18** | **左列・下9番目** | **下段・左9番目** | **青** | 表示 |
| **DIN** | **19** | **右列・下10番目** | **上段・左10番目** | **緑** | 表示 |
| **DC** | **22** | **左列・下11番目** | **下段・左11番目** | **黄** | 表示 |
| **CLK** | **23** | **右列・下12番目** | **上段・左12番目** | **白** | 表示 |
| **CS** | **24** | **左列・下12番目** | **下段・左12番目** | **青** | 表示 |
| **UP** | **31** | **右列・下16番目** | **上段・左16番目** | **茶** | 操作 |
| **DOWN** | **35** | **右列・下18番目** | **上段・左18番目** | **茶** | 操作 |
| **LEFT** | **29** | **右列・下15番目** | **上段・左15番目** | **茶** | 操作 |
| **RIGHT** | **37** | **右列・下19番目** | **上段・左19番目** | **茶** | 操作 |
| **PRESS** | **33** | **右列・下17番目** | **上段・左17番目** | **茶** | 操作 |
| **KEY1** | **40** | **左列・下20番目** | **下段・左20番目** | **橙** | 操作 |
| **KEY2** | **38** | **左列・下19番目** | **下段・左19番目** | **橙** | 操作 |
| **KEY3** | **36** | **左列・下18番目** | **下段・左18番目** | **橙** | 操作 |

※5V電源は、ラズパイの2番ピンから直接ではなく、**IC（74AHCT125）の14番ピンから分岐** させるとスマートです。

> [!IMPORTANT]
> **レイアウトの選択：スタッキング vs 横並び**
> 本プロジェクトで「横並び構成」を推奨した理由は、高負荷時の **「熱対策」** と **「メンテナンス性」** です。しかし、配線の手間については大きな差があります。
>
> | 特徴 | スタッキング (2枚重ね) | 横並び (本ドキュメント推奨) |
> | :--- | :--- | :--- |
> | **配線** | ほぼ不要 (ピンを刺すだけ) | **17本のブリッジ配線が必要** |
> | **熱対策** | 熱がこもりやすい (要注意) | 極めて良好 (大型基板で放熱) |
> | **サイズ** | 最小・コンパクト | 横に広がる |
> | **難易度** | 低 (ただし長いピンが必要) | 中 (配線作業がメイン) |
>
> もし「17本の配線は多すぎる、やはりコンパクトにしたい」と感じる場合は、スタッキング構成への切り替えも検討してください。その場合、ラズパイに「長いピンソケット」を立てて LCD を上に載せる形になります。

---

## 4. 総点検（テスターがある場合）

電源を入れる前に、テスターの「導通モード」で以下の点を確認してください。

- [ ] 5V と GND が繋がっていないか（ショートしていないか）。
- [ ] GPIO 18 から IC の Pin 2 まで電気が通っているか。
- [ ] IC の Pin 1 が確実に GND に落ちているか。

---

## 🏁 完了チェックリスト

- [ ] レベルシフタ IC の向き（欠けがある方が上）が正しい。
- [ ] 330Ω の抵抗が信号線に割り込んでいる。
- [ ] Pi と LCD が干渉せず、しっかり固定されている。

次は、ACアダプタと安全のための保護回路の作成に進みます。

[**フェーズ 2.3: 電源システムと安全回路**](hw_step2_3_power_safety.md) へ進む ➡️
